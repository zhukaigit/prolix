...
//    GridFSFile gfsFile = gridFsTemplate.findOne(Query.query(Criteria.where("_id").is("5c45a20111d83807f254eb2f")));
    GridFSFile gfsFile = gridFsTemplate.findOne(Query.query(new Criteria()));

    // 查询单个文件
    GridFsResource gridFsResource = new GridFsResource(gfsFile);
    InputStream inputStream = gridFsResource.getInputStream();

    System.out.println("文件字符串如下：\n" + StreamUtils.copyToByteArray(inputStream).length);
  }
